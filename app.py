...
try:
    with open('config.json') as f:
        config = json.load(f)
    AIO_USERNAME = config['adafruit_io']['username']
    AIO_KEY = config['adafruit_io']['api_key']
    DB_CONNECTION_STRING = config.get('database', {}).get('connection_string', '')
except FileNotFoundError:
    # Fallback to environment variables
    AIO_USERNAME = os.environ.get('AIO_USERNAME', 'YOUR_USERNAME')
    AIO_KEY = os.environ.get('AIO_KEY', 'YOUR_KEY')
    DB_CONNECTION_STRING = os.environ.get('DB_CONNECTION_STRING', '')

# Adafruit IO API endpoints
AIO_BASE_URL = 'https://io.adafruit.com/api/v2'

# Feed names (update these to match your actual feed names)
FEEDS = {
    'temperature': f'{AIO_USERNAME}/feeds/temperature',
    'humidity': f'{AIO_USERNAME}/feeds/humidity',
    'motion': f'{AIO_USERNAME}/feeds/motion',
    'light': f'{AIO_USERNAME}/feeds/light',
    'fan': f'{AIO_USERNAME}/feeds/fan',
    'mode': f'{AIO_USERNAME}/feeds/mode',
    'camera': f'{AIO_USERNAME}/feeds/camera'
}
